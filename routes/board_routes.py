...
def create_task_route(board_id):
    ...
    try:
        ...
        # Handle due_date - convert to string if it's a date object
...
@boards_bp.route("/<int:board_id>/tasks/<int:task_id>", methods=["GET"])
@login_required
...
@boards_bp.route("/<int:board_id>/tasks/<int:task_id>/move", methods=["POST"])
@login_required
def move_task_route(board_id, task_id):
    ...
    if user_role not in ['owner', 'editor']:
        return jsonify(error="Permission denied - only owners and editors can move tasks"), 403
