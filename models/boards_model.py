    ...
    with get_db() as conn:
        with conn.cursor(dictionary=True) as cur:
            for status in statuses:
                cur.execute(
                    """
                    SELECT t.id, t.title, t.assigned_to, t.due_date, t.status, t.created_at,
                           u.username as assigned_username
                    FROM tasks t
                    LEFT JOIN users u ON t.assigned_to = u.id
                    WHERE t.board_id = %s AND t.status = %s
...
def create_task(
    ...
    with get_db() as conn:
        with conn.cursor(dictionary=True) as cur:
            cur.execute(
                """
                INSERT INTO tasks (board_id, title, assigned_to, due_date, status, created_by, created_at)
                VALUES (%s, %s, %s, %s, %s, %s, NOW())
                """,
                (board_id, title, assigned_to, due_date, status, created_by)
            )
            conn.commit()
